...
class UserCenterController extends AddonsController {
	...
	public function lists() {
		...
		foreach ( $data as $k => $d ) {
			...
			$user ['signs'] =  implode('<br>', $this->getUserSigns($d['uid'], $signList));
			$data [$k] = $user;
		}
		/* 查询记录总数 */
	...
	// 获得所有活动
	function getSignList($token){
		$map ['token'] = get_token ();
		$signs = M ( 'sign' )->where ( $map )->getFields ( 'id,title' );
		return $signs;
	}
	// 获得用户参加的活动
	function getUserSigns($uid, $signList){
		$map ['uid'] = $uid;
		$userSigns = M ( 'sign_value' )->where ( $map )->getFields ( 'sign_id, is_pay, price' );
		$userSignsSting = array();
		foreach ( $userSigns as $sign ) {
			if($signList[$sign['sign_id']]){
				if($sign['is_pay'] == 1){
					if($sign['price'] > 0){
						$is_pay = '<span class="icon-pay2">现金支付 ￥' . $sign['price'] . '</span>';
					} else {
						$is_pay = '<span class="icon-pay1">邀请免费报名</span>';
					}
				} else {
					$is_pay = '<span class="icon-pay0">未完成</span>';
				}
				$userSignsSting[] = $signList[$sign['sign_id']] . '<' .$is_pay . '>';
			}
		}
		return $userSignsSting;
	}
