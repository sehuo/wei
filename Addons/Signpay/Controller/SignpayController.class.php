...
use Addons\Signpay\Controller\BaseController;

class SignpayController extends BaseController {
	...
	public function __construct() {
		parent::__construct ();
		$this->model = M ( 'Model' )->getByName ( 'signpay_set' );
	...
	public function lists($type = 0) {
		...
		// 获取模型信息
		$payid = M ( "signpay_set" )->where ( array (
				"token" => $token 
		) )->field ( 'id' )->find ();
		$id = 0;
		if (! empty ( $payid )) {
	...
	public function testpay() {
		if (IS_POST) {
			 ...
			 */
			$from = "Signpay:__Signpay_playok";
			$bid = "";
			$sid = "";
			$url = addons_url ( 'Signpay://Alipay/pay', array (
					'from' => $from,
					'orderName' => $orderName,
					'price' => $price,
					'token' => $token,
	...
	//         'paytype' =>0
	//     );
	//     $url=addons_url('Signpay://Alipay/pay',$param);
	//     $this->assign('url',$url);
