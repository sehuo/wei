...
class QrCodeModel extends Model {
	...
	function add_qr_code($action_name = 'QR_SCENE', $addon = '', $aim_id = '', $extra_int = '', $extra_text = '') {
		...
		$where ['extra_text'] = $extra_text;
		$where ['extra_int'] = $extra_int;

		$qrcode = $this->get_same_qrcode($where);//返回已存在场景二维码路径
		if($qrcode)
		{
			logs("[get_same_qrcode] where=". json_encode($where));
			return $qrcode;
		}else{
			$data ['qr_code'] = $this->QrcodeCreate ( $data ['scene_id'], $data ['action_name'] ,$extra_text);
			logs("[QrcodeCreate] agr=[data=" . $data['scene_id'] ." action_name=$action_name extra_text=$extra_text] data=". json_encode($data));
			if (! $data ['qr_code']) {
				logs("[QrcodeCreate] ", 'error');
				return - 2; // 获取二维码失败
			}
			
			$res = $this->add ( $data );
			if (! $res) {
				logs("[QrcodeCreate add] ", 'error');
				return - 3; // 保存数据失败
			}
			return $data ['qr_code'];
		}
	...
	private function QrcodeCreate($qrcodeID, $qrcodeType = 'QR_SCENE',$extra_text) {
		if ($qrcodeType == 'QR_LIMIT_SCENE') {
			$tempJson = '{"action_name": "' . $qrcodeType . '", "action_info": {"scene": {"scene_id": ' . $qrcodeID . '}}}';			

		}else if($qrcodeType == 'QR_LIMIT_STR_SCENE'){

			if($extra_text){
				//$tempJson = '{"action_name": "' . $qrcodeType . '", "action_info": {"scene": {"scene_str": ' . $extra_text . '}}}';
	...
	/**
	 * [get_same_qrcode 获取相同的场景二维码]
	 * @return [type] [description]
	 */
	private function get_same_qrcode($where)
	{
		$result = M('qr_code')->where($where)->find();
