...
function logs($logstr='', $type='') {
	$ymd = @date('Y-m-d');
	if($type == 'error'){
		$logFile = './logs/error-' . $ymd . '.txt';
	}else{
		$logFile = './logs/info-' . $ymd . '.txt';
	}

	$script_name = pathinfo($_SERVER['PHP_SELF'], PATHINFO_FILENAME);
	// $query_string = preg_replace('/^s=/','',$_SERVER['QUERY_STRING']);
	$time = @date('[H:i:s] ');
	
	$log_string = "$time [$script_name] $logstr \r\n";
	file_put_contents($logFile, $log_string ,FILE_APPEND);
}
